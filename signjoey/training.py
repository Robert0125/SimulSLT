...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.early_stop = train_config.get("early_stop", True)
        self.wait_k = config["model"]["wait_k"]
        self.wait_k_method = config["model"]["wait_k_method"]
    ...
    def init_from_checkpoint(
        ...
        # only load pretrain segmentation
        model_dict = self.model.state_dict()
        for k, v in model_dict.items():
            if (
                "seg_encoder" in k
                or "seg_sgn_embed" in k
                or "seg_gloss_output_layer" in k
            ):
                new_k = k.replace("seg_encoder", "encoder")
                new_k = new_k.replace("seg_sgn_embed", "sgn_embed")
                new_k = new_k.replace("seg_gloss_output_layer", "gloss_output_layer")
                model_dict[k] = model_checkpoint["model_state"][new_k]
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        epoch_no = None
        # copy_seg_encoder = copy.deepcopy(self.model.seg_encoder)
        # copy_seg_sgn_embed = copy.deepcopy(self.model.seg_sgn_embed)
        # copy_seg_gloss_output_layer = copy.deepcopy(self.model.seg_gloss_output_layer)
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                (
                    ...
                    nar_loss,
                    boundary_loss,
                ) = self._train_batch(batch, update=update)

                # for cp, now in zip(
                #     copy_seg_encoder.parameters(), self.model.seg_encoder.parameters()
                # ):
                #     if not torch.equal(cp.data, now.data):
                #         print("fucking change")
                #         exit(1)
                # for cp, now in zip(
                #     copy_seg_sgn_embed.parameters(),
                #     self.model.seg_sgn_embed.parameters(),
                # ):
                #     if not torch.equal(cp.data, now.data):
                #         print("fucking change")
                #         exit(1)
                # for cp, now in zip(
                #     copy_seg_gloss_output_layer.parameters(),
                #     self.model.seg_gloss_output_layer.parameters(),
                # ):
                #     if not torch.equal(cp.data, now.data):
                #         print("fucking change")
                #         exit(1)
                ...
                if self.wait_k_method == "fire":
                    self.tb_writer.add_scalar(
                        "train/train_nar_loss", nar_loss, self.steps
                    )
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if (
                        ...
                        if self.early_stop and prev_lr != now_lr:
