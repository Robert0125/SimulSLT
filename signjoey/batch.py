...
class Batch:
    ...
    def __init__(
        ...
        if is_train and torch_batch.boundary[0] != "":
            self.boundary = torch_batch.boundary
            self.boundary = self.sgn.new_tensor(
                [int(j) for i in self.boundary for j in i.split()]
            )
        else:
            self.boundary = None
        # Here be dragons
        ...
        if self.boundary is not None:
            assert self.sgn_mask.sum() == len(self.boundary), "我要崩溃了"
